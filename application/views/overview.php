<div class="jumbotron" style="background-color:#460d5c; color:white;">
  <div class="container">
    <div class="row">
      <div class="col-12">
        <h2>ตารางสถิติจำนวนลูกค้าค้างชำระค่าไฟฟ้าของ กฟต.1 ทุกเขต</h2>
      </div>
    </div>
    <div class="row">
      ...
      <form method="GET" id="searchfrom" name="searchfrom" action="">
        <ul class="search">
          ...
          <li>
						<button type="button" class="btn btn-round langmenu btn-border border-white" onclick="exportexceloverview()">ดาวน์โหลดเป็นไฟล์ Excel</button>	
					</li>
        </ul>
      </form>
      </div>
  ...
  <div class="container">
    <div class="row justify-content-center" style="margin-bottom:10px;">
      <div class="col-12">
        <h4 align="center"><?php echo 'ตารางสถิติจำนวนลูกค้าค้างชำระค่าไฟฟ้าของ กฟต.1 ทุกเขต ในรอบ ' . $formselected['amountmonth_name'] . ' (' . $dateoutput['th_startmonth'] . ' ' . $dateoutput['startyear'] ?><?php echo ($dateoutput['th_endmonth'] == $dateoutput['th_startmonth'] && $dateoutput['startyear'] == $dateoutput['endyear']) ? ')' : ' - ' . $dateoutput['th_endmonth'] . ' ' . $dateoutput['endyear'] .')'?></h4>
      </div>
    </div>
  </div>
  <table class="table table-striped table-bordered" id="example" class="display" style="width:100%">
    <thead class="thead-dark">
      <tr>
        ...
        ?>
        <th>รายชื่อค้างชำระในเขต</th>
      </tr>
    </thead>
    <tbody>
      <?php
      foreach ($stat as $index => $info) { ?>
        <tr>
          <td><?php echo $info['trsg_id']?></td>
          <td><?php echo $info['trsg_name']?></td>
          <td><?php echo $info['ผ่อนผันครั้งที่ 1']?></td>
          <td><?php echo $info['ผ่อนผันครั้งที่ 2']?></td>
          <td><?php echo $info['ปลดสาย']?></td>
          <td><?php echo $info['ถอดมิเตอร์']?></td>
          <td><a href="<?php echo site_url('search/searchbytrsg' . '?trsgform='. $info['trsg_id'] . '&amountmonthform='. $formselected['amount'] . '&statusform=all')?>">รายชื่อ</a></td>
        </tr>
      <?php
      }?>
    </tbody>
